...
:::note
Ada is available on different programming languages such as **C++**, **Python**, **Go**, **Rust**, **Kotlin** or **LuaJIT**. 
:::
...
Ada provides the following CMake options to customize the build:

- `ADA_TESTING`: Enable tests (requires git). Default: `OFF`.
...
You may amalgamate all source files into only two files (`ada.h` and `ada.cpp`) by executing the Python
